...
class PlayCommand(private val main: Apocalypse) : Command("play", "Начать игру", "/play") {
    ...
    private fun execute(player: Player){
        ...
        ).addButton("Случайное появление", ImageType.PATH, "textures/ui/dragonestia/server") { _, _ -> spawn(player) }

        if(data.hasBed()){
            form.addButton("Появление на спальном мешке", ImageType.PATH, "textures/ui/dragonestia/server") {_, _ ->
                player.teleport(data.bed.position)
...
class PlayerSpawnTask(owner: Apocalypse, val player: Player) : PluginTask<Apocalypse>(owner) {
    ...
    override fun onRun(tick: Int) {
        ...
        val data = owner.playerManager.get(player)
