...
public class MainListener implements Listener {
    ...
    public void onQuit(PlayerQuitEvent event){
        ...
        main.getPlayerManager().unload(player);
    }
    ...
    public void onDeath(PlayerDeathEvent event){
        ...
        if(player.getLastDamageCause() instanceof EntityDamageByEntityEvent){
            ...
            if(damageEvent.getDamager() instanceof Player){
                ...
                PlayerData killerData = main.getPlayerManager().get(killer);

                killerData.onKill(data);
            }
        }

        player.sendTitle("§c§l"+deathMessages[random.nextInt(deathMessages.length)], "Всего вы умерли уже " + data.getStats().getDeaths() + " раз", 20, 20 * 3, 20);
